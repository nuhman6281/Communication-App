...
export function MessageContentRenderer({
    ...
    return (
      <div className="space-y-2">
        {files.map((file, idx) => {
          ...
          const displayName =
            file.fileName?.split("-").slice(5).join("-") ||
            file.fileName ||
            "Unknown file";
              ...
              <div
                className={cn(
                  ...
                  mimeType.includes("pdf")
                    ...
                    : mimeType.includes("doc") || mimeType.includes("word")
    ...
    switch (message.messageType) {
      case "text":
        const markdownComponents: Components = {
          ...
          // CODE BLOCK (triple backtick ```) - DARK TERMINAL WITH GREEN ACCENT
          pre: ({ children }) => (
            <div
              className={cn(
            ...
            >
              ...
              >
                ...
                <div className="text-[9px] px-2 py-0.5 bg-emerald-600/30 rounded">
                  SYNTAX
                </div>
              </div>
              <pre
      ...
      case "image":
        const images =
          message.metadata?.files?.filter((f) =>
          ...
          <div className="space-y-2">
            {files.map((file, idx) => {
              ...
              return (
                <div
                  key={idx}
                  className="flex items-center gap-3 p-3 bg-muted/50 rounded-lg max-w-sm hover:bg-muted/70 transition-colors"
                >
                  <div
                    className={cn(
  ...
  return (
    <div className={cn("max-w-full", className)}>{renderMessageContent()}</div>
  );
}
